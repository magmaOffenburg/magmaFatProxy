...
public class ServerMessageParser implements IMessageParser
{
	...
	public Map<String, IPerceptor> parseMessage(byte[] message) throws PerceptorConversionException
	{
		...
		Map<String, IPerceptor> map = new HashMap<>();
	...
	private void parseNode(SymbolNode node, Map<String, IPerceptor> map) throws PerceptorConversionException
	{
		...
		case "FRP":
			...
			break;
		case "proxy":
			perceptor = parseProxyCommand(node);
			break;
		default:
			/* Unknown node? */
			break;
	...
	private void parseLine(SymbolNode node, Map<String, IPerceptor> map, int lineID) throws PerceptorConversionException
	{
		try {
			...
			LinePerceptor perceptor = new LinePerceptor(name + lineID, pol1, pol2, true);
			map.put(perceptor.getName(), perceptor);
	...
	private void parsePlayer(SymbolNode node, Map<String, IPerceptor> map) throws PerceptorConversionException
	{
		try {
			...
			PlayerPos perceptor = new PlayerPos(name, pol, id, teamName, bodyPartMap, true);
	...
	private void parseVisibleObject(SymbolNode node, Map<String, IPerceptor> map) throws PerceptorConversionException
	{
		try {
			...
			VisibleObjectPerceptor perceptor = new VisibleObjectPerceptor(name, pol, true, 1.0);
			map.put(perceptor.getName(), perceptor);
		} catch (IndexOutOfBoundsException e) {
			throw new PerceptorConversionException("Malformed node: " + node.toString());
		}
	...
	/**
	 * Parse a symbol tree node into a ProxyPerceptor object
	 *
	 * @param node Symbol tree node
	 * @return ProxyPerceptor object
	 * @throws PerceptorConversionException If the input string contains illegal
	 *         data which cannot be converted
	 */
	private ProxyPerceptor parseProxyCommand(SymbolNode node) throws PerceptorConversionException
	{
		try {
			/* Get the name of the command */
			ProxyPerceptor perceptor = new ProxyPerceptor((String) node.children.get(1));

			// Get the values
			int size = node.children.size() - 2;
			float[] values = new float[size];
			for (int i = 0; i < size; i++) {
				values[i] = parseFloat(node.children.get(i + 2));
			}
			perceptor.setValues(values);
